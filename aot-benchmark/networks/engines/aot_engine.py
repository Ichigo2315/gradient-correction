...
class AOTEngine(nn.Module):
    ...
    def add_reference_frame(self,
        ...
        # Gradient correction: allow a (possibly soft / differentiable) one-hot
        # mask to enter the id-embedding path directly, bypassing the
        # non-differentiable hard comparison in one_hot_mask().
        if one_hot_override is not None:
            curr_one_hot_mask = one_hot_override
...
class AOTInferEngine(nn.Module):
    ...
    def add_reference_frame_soft(self, img, soft_one_hot, obj_nums,
                                 frame_step=-1):
        """Add a reference frame whose guidance mask is a *soft* one-hot
        probability map ``[1, max_obj_num+1, H, W]`` that requires grad.

        Used by the gradient-correction module so that the cycle prediction is
        differentiable w.r.t. the reference mask (paper Eq.(5)). Only supports a
        single underlying AOT engine, i.e. ``obj_nums <= max_aot_obj_num``
        (always true on DAVIS-2017, where at most 10 objects appear).
        """
        if isinstance(obj_nums, list):
            obj_nums = obj_nums[0]
        assert obj_nums <= self.max_aot_obj_num, (
            "add_reference_frame_soft only supports a single AOT engine "
            f"(obj_nums={obj_nums} > max_aot_obj_num={self.max_aot_obj_num})")
        self.obj_nums = obj_nums

        if len(self.aot_engines) == 0:
            new_engine = AOTEngine(self.AOT, self.gpu_id,
                                   self.long_term_mem_gap,
                                   self.short_term_mem_skip)
            new_engine.eval()
        ...
        self.aot_engines[0].add_reference_frame(
            img,
            mask=None,
            obj_nums=[obj_nums],
            frame_step=frame_step,
            one_hot_override=soft_one_hot)
        self.update_size()
