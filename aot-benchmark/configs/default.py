...
class DefaultEngineConfig():
    def __init__(self, exp_name='default', model='aott'):
        ...
        # Test-time gradient correction (Li et al., NeurIPS 2020).
        # Turn off to obtain a clean no-GC baseline under the same code path.
        self.TEST_GRADIENT_CORRECTION = True
        self.TEST_GC_INTERVAL = 5     # K: apply correction once every K frames
        self.TEST_GC_ITER = 10        # N: gradient-descent steps per correction
        self.TEST_GC_RATE = 20        # alpha: correction (step) rate
        # Adaptive (uncertainty-gated) step: freeze confident pixels, correct
        # only uncertain ones. With this on, TEST_GC_RATE acts as alpha_max.
        self.TEST_GC_ADAPTIVE = False
        self.TEST_GC_ADAPT_GAMMA = 1.0  # entropy-weight sharpening exponent
        self.TEST_GC_LEGACY = False     # use original (legacy) GC implementation
